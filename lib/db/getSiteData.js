...
function handleDataBeforeReturn(db) {
  ...
  // Sanitize undefined fields to null to prevent Next.js serialization errors
  db.allPages = db.allPages.map(page => {
    const sanitized = {}
    for (const key of Object.keys(page)) {
      sanitized[key] = page[key] === undefined ? null : page[key]
    }
    return sanitized
  })
